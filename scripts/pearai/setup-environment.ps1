...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\closedai-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\closedai-ref' -> 'extensions\closedai-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\closedai-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\closedai-ref'
