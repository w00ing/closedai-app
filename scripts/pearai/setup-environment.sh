...
app_dir=$(pwd)
target_path="$app_dir/extensions/closedai-submodule/extensions/vscode"
link_path="$app_dir/extensions/closedai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/closedai-submodule" ]; then
    echo "Removing existing closedai-submodule directory"
    execute "rm -rf $app_dir/extensions/closedai-submodule" "Failed to remove existing closedai-submodule directory"
fi
...
execute "cd ./extensions/closedai-submodule" "Failed to change directory to extensions/closedai-submodule"
